 ...
 */
export function checkBirthday(server, rem) {
  setTimeout(() => {
    ...
    birthdayUsers.forEach(async userArr => {
      ...
      if (userMonth == currentMonth && userDate == currentDate) {
        ...
        const picture = new AttachmentBuilder('https://i.imgur.com/7IqikPC.jpg');
 ...
 */
export function checkHoliday(channels) {
  setTimeout(async () => {
    // get today's month and date
...
export function secsToMidnight() {
